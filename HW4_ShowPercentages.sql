...
BEGIN
    ...
    WITH Percentages AS
        ...
        ON R.AName = A.AName)
    SELECT S.SID, S.LName, S.FName, S.Sec, R.AName, P.Percent--, ROUND(( (SUM(Quizzes.Score) / COUNT(Quizzes.Score)) * 0.4 + (SUM(Exams.Score) / COUNT(Exams.Score)) * 0.6 ), 2) AS CourseAvg
