...
BEGIN
    ...
    --     ON R.AName = A.AName)
    -- SELECT S.SID, S.LName, S.FName, S.Sec, R.AName, ROUND((100 * IFNULL(R.Score, 0) / PtsPoss), 2) AS Grade, ROUND(( (SUM(Quizzes.Score) / COUNT(Quizzes.Score)) * 0.4 + (SUM(Exams.Score) / COUNT(Exams.Score)) * 0.6 ), 2) AS CourseAvg
